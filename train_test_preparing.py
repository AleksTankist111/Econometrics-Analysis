...
def train_test_split(X, y, test_size = 0.2, equal = False, shuffle = False, random_seed = None):
    ...
    else: n_trs = int(len(y)*(1-test_size))
    ...
    if not equal:
        ...
        for k, v in labels.items():
            ...
            X_train.append(X[v[:n_trs]])
            X_test.append(X[v[n_trs:]])
            y_train.append(y[v[:n_trs]])
            y_test.append(y[v[n_trs:]])
    ...
    if supplement:
        ...
        for key, val in labels.items():
            ...
            if X_new == None:
                ...
                y_new = np.row_stack((y_new,y[idxs]))
